(function(){
  ...
  subapp.get('/', function(req, res){
      ...
      function(err, db){
       ...
       if(err) {
         ...
         res.status(500).render('index', {
           error: 'Kunde inte ladda in recepten'
         });
       }
       else {
    ...
    var
      database = subapp.get('database'),
      s   = req.query.q;
    ;
    if(typeof s == 'undefined')             res.status(204).send('Ingen information');
    else if(typeof database == 'undefined') res.status(500).send('Kan inte ladda in recepten');
    else {
      var results = query(['recept[*associeringar~/^'+s+'/i|titel~/^'+s+'/i|kategori~/^'+s+'/i|associeringar~?|titel~?|kategori~?]', s, s, s], { 
            allowRegexp: true,
            data: database,
            force: []
      }).value;
